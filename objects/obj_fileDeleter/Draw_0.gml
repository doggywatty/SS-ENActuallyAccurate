...
var _string = string($"[c_white][{sprite_get_name(_sprite)},{image_index}][spr_font][c_red][fa_center][fa_middle]{lang_get(_text, [_filenum])}[c_white][{sprite_get_name(_sprite)},{image_index}]\n[{(optionSelection == 1) ? "c_gray" : "c_white"}]{lang_get("main_menu_yes")}         [{(optionSelection == 0) ? "c_gray" : "c_white"}]{lang_get("main_menu_no")}");
