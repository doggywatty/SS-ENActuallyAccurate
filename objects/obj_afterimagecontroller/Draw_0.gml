for (var i = 0; i < ds_list_size(global.afterimage_list); i++)
{
	...
	with (b)
	{
		...
		if (visible)
		{
			...
			if (color_choose != afterimagetypes.basic)
			{
				...
				if (color_choose == afterimagetypes.palette && instance_exists(identity) && object_is_ancestor(identity.object_index, obj_parent_player))
				{
					with (identity)
					{
