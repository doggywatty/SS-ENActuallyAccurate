...
function scr_calculate_farParallaxX(_spr, _mult)
{
	...
	return max(sprite_size * max_pos * _mult, 0);
}

function scr_calculate_farParallaxY(_spr, _mult)
{
	if (!sprite_exists(_spr))
		return 0;
...
function scr_addParallaxLayer(_layer, _func, _args = [])
{
	ds_map_set(global.ParallaxMap, _layer, 
	{
		func: _func,
		args: _args
	});
...
function defaultParallaxZigZag(_x, _y, _x1, _x2)
{
	var time = global.CurrentTime;
	x += (camera_get_view_x(view_camera[0]) * _x);
	y += (camera_get_view_y(view_camera[0]) * _y);
	x += wave(-_x1, _x1, 4, 10, time);
	y += wave(-_x2, _x2, 4, 10, time);
}
