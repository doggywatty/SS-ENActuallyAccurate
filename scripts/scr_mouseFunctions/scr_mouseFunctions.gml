function get_mouse_x_screen(_pos = 0)
{
	var mouse_xpos = device_mouse_x_to_gui(_pos);
	var params = calculate_letterbox_params();
	return round((mouse_xpos - params.screen_x) / params.scale);
}

function get_mouse_y_screen(_pos = 0)
{
	var mouse_ypos = device_mouse_y_to_gui(_pos);
	var params = calculate_letterbox_params();
	return round((mouse_ypos - params.screen_y) / params.scale);
}

function get_mouse_x(_pos = 0)
{
	var mouse_xpos = get_mouse_x_screen(_pos);
...
function get_mouse_y(_pos = 0)
{
	var mouse_ypos = get_mouse_y_screen(_pos);
