...
function scr_check_joysticks(_device, _spr_icon = 0.5)
{
...
function scr_checkanystick(_device, _spr_icon = 0.5)
{
	var sticks = [gp_axislh, gp_axislv, gp_axisrh, gp_axisrv];
	...
	for (var i = 0; i < array_length(sticks); i++)
	{
		...
		if (val > _spr_icon || val < -_spr_icon)
			return true;
	}
...
function scr_input_icon_add(_inp, _spr, _ind_icon)
{
	for (var i = 0; i < array_length(_inp); i++)
	{
		var input = _inp[i];
		ds_map_set(global.input_icons, input, [_spr, _ind_icon]);
		trace("Added ", sprite_get_name(_spr), $" (frame: {_ind_icon}) to input icon map for {input}.");
	}
}
...
function get_control_sprite(_inputkey, _spr_icon = false)
{
